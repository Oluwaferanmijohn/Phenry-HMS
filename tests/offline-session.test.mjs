...
test('profile fetch failures are distinct from confirmed revocation', async () => {
  ...
  assert.match(middleware, /offline && profile\.value\?\.active/)
})
...
test('offline identity transitions preserve encrypted local data', async () => {
  ...
  assert.match(sync, /navigator\.onLine && previousUserId && previousUserId !== userId/)
  assert.match(sync, /if \(!navigator\.onLine && !userId\) return/)
  assert.match(queue, /user\.value\?\.id \|\| \(!state\.online \? state\.hydratedFor/)
  assert.match(recentPatients, /user\.value\?\.id \|\| \(!online\.value \? profile\.value\?\.id/)
