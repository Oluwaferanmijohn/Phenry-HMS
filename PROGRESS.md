...
- [x] **Doctor had no Surgery & Procedures page; Chief Embryologist and Lab Tech had zero visibility into it at all** — new pages for all three, reusing the existing shared `SurgeryPage.vue` (Doctor: full scheduling access, now widened to see every procedure clinic-wide rather than only their own — matches "assigned to him or not," with a "You" badge on their own cases; Embryologist/Lab Tech: read-only awareness, `canDocument=false` since operative reports stay a Doctor/Matron/Nurse responsibility). **Self-correction**: while building this, briefly "fixed" `ScheduleProcedureModal.vue`/`SurgeryPage.vue`/`matron/overview.vue` to use `assigned_doctor_id` instead of `assigned_provider_id`, based on reading migration 006's `CREATE TABLE` in isolation — missed that migration 007 deliberately renames that column right after (documented there: Matron can be primary provider too, not just Doctor). Broke live surgery scheduling for one round until caught via the actual `PGRST204` error and a direct `information_schema.columns` check against the real database. Reverted; `assigned_provider_id` was correct all along.
