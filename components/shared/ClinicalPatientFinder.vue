<template>
  <div class="finder">
    <div class="finder-controls">
      <select v-model="mode" class="input finder-mode" aria-label="Search by"><option value="all">All fields</option><option value="name">Name</option><option value="mrn">Patient ID</option><option value="phone">Phone</option></select>
      <input v-model="query" class="input finder-query" placeholder="Search patient name, ID, or phone…" @focus="open = true" />
      <input v-model="date" class="input finder-date" type="date" title="Registered or appointment date" />
    ...
    <div v-if="open" class="finder-results card">
      ...
      <button v-for="entry in entries" :key="entry.patient_id" class="finder-result" type="button" @click="choose(entry)">
        ...
        <span class="finder-meta"><small v-if="entry.appointment_date">Visit {{ entry.appointment_date }}</small><small v-else>Registered {{ entry.registered_on }}</small><Badge v-if="entry.is_walk_in" tone="amber">Walk-in</Badge></span>
      </button>
...
const props = withDefaults(defineProps<{ modelValue?: string; allowWalkIn?: boolean }>(), { modelValue: '', allowWalkIn: false })
...
const mode = ref('all'); const query = ref(''); const date = ref(''); const entries = ref<any[]>([]); const loading = ref(false); const open = ref(false); const showWalkIn = ref(false); const creating = ref(false)
const walkIn = reactive({ firstName: '', surname: '', dob: '', sex: '', phone: '', reason: '' })
let timer: ReturnType<typeof setTimeout> | undefined
async function search() { loading.value = true; const { data, error } = await supabase.rpc('clinical_patient_search', { p_query: query.value.trim(), p_mode: mode.value, p_date: date.value || null, p_status: null, p_limit: 50 }); loading.value = false; if (error) { entries.value = []; toast('Patient search is unavailable until the clinical workflow database update is installed.', 'warn'); return }; entries.value = data || [] }
watch([query, mode, date], () => { open.value = true; clearTimeout(timer); timer = setTimeout(() => void search(), 220) }, { immediate: true })
